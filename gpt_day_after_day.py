...
def calculate_profit_loss(trade_direction, entry_price, exit_price, position_size):
    number_of_shares = position_size / entry_price if entry_price != 0 else 0
...
# ------------------------------------------------
# 3) Precompute All Potential Trades
# ------------------------------------------------
def precompute_trades(df, vix_df, initial_investment):
    """
    Creates a DataFrame of all valid gap trades:
     - One row per day that meets the gap criteria.
    """
    trades = []
    total_days = len(df)

    # Build a dictionary: current_date -> VIX open (from the previous day)
    ...
    for i_day in range(1, total_days):
        ...
        trades.append({
            ...
            'df_idx': i_day  # store which day index in df
        })
...
def optimize_drop_filter_by_success_rate(trades_df, df, drop_ratios, vix_date_index,
    ...
    # Put into DataFrame and sort by success rate desc, then occurrence count, then profit
    ...
    if not results_df.empty:
        ...
        best_Y = top_row['Y']
        best_Z = top_row['Z']
        best_occ = top_row['OccurrenceCount']
        best_succ = top_row['SuccessCount']
    ...
    return (best_X, best_Y, best_Z, best_sr, best_pnl, best_occ, best_succ, results_df)
...
def optimize_atr_parameters(trades_df, df, n_values, T_values):
    ...
    # Sort primarily by total profit (you can change to success rate if desired)
...
# ------------------------------------------------
# 8) Consecutive Success Analysis
# ------------------------------------------------
def analyze_consecutive_success(trades_df, max_streak=5):
    """
    Analyzes consecutive 'success' days among the filtered trades.
    Success here is defined as Profit/Loss > 0.
    For each streak length k (1..max_streak):
       - count how many times we have k consecutive successful trades
       - compute P(k-th day success | (k-1) consecutive successes)
    Then prints a summary table.
    """
    if trades_df.empty:
        print("No trades available for consecutive success analysis.")
        return

    # Sort by date to ensure chronological order
    trades_df = trades_df.sort_values(by='Date').reset_index(drop=True)
    success = (trades_df['Profit/Loss'] > 0).values
    n = len(success)

    # counts[k] = how many times we have a streak of length k starting at index i
    # i.e. success[i], ..., success[i+k-1] are all True
    counts = [0] * (max_streak + 1)

    for i in range(n):
        for k in range(1, max_streak + 1):
            if i + k - 1 < n:
                if all(success[i : i + k]):  # success[i..i+k-1]
                    counts[k] += 1
            else:
                break

    print("\n=== CONSECUTIVE SUCCESS ANALYSIS ===")
    print("Total trades considered:", n)
    print("\nStreak | #Occurrences | Probability (conditional on previous streak)")
    print("---------------------------------------------------------------")

    # For k=1, probability is just success rate out of all trades
    for k in range(1, max_streak + 1):
        if k == 1:
            # Probability that a day is successful (out of total trades)
            prob = counts[1] / n if n > 0 else 0
            print(f"{k:5d} | {counts[k]:11d} | {prob:>10.2%} (vs. all days)")
        else:
            # Probability that we have k consecutive successes
            # given we had (k-1) consecutive successes
            denom = counts[k - 1]
            if denom > 0:
                prob = counts[k] / denom
            else:
                prob = 0
            print(f"{k:5d} | {counts[k]:11d} | {prob:>10.2%} (given streak={k-1})")

# ------------------------------------------------
# 9) Main Combined Function
# ------------------------------------------------
def run_combined_drop_vix_atr(
    ...
    Y_values=np.arange(1.0, 6.0, 1.0),  # floats that will become int in apply_drop_filter
    Z_values=np.arange(1.0, 6.0, 1.0),  # same
    min_occurrence_drop=0,   # optional min # trades for drop filter
    # VIX optimization
    vix_i_min=10.0, 
    vix_i_max=30.0, 
    vix_j_min=10.0, 
    ...
    atr_T_max=15.0, 
    atr_T_step=1.0,
    # Consecutive success streak limit
    max_streak=5
):
    ...
    # 2) Precompute All Gap Trades
    print("\nPrecomputing all potential gap trades...")
    trades_df = precompute_trades(df, vix_df, initial_investment)
    if trades_df.empty:
        print("No valid gap trades. Exiting.")
        return
    print(f"Total gap trades found: {len(trades_df)}")
    ...
    final_trades = trades_best_vix.merge(df_atr, on='Date', how='left')
    ...
    if final_trades.empty:
        ...
        # 7) Consecutive Success Analysis
        analyze_consecutive_success(final_trades, max_streak=max_streak)


# ------------------------------------------------
# Example Usage (You can comment this out if you want)
# ------------------------------------------------
if __name__ == "__main__":
    ...
    run_combined_drop_vix_atr(
        ...
        atr_T_max=15.0,
        atr_T_step=1.0,
        max_streak=5  # Up to 5-day consecutive success analysis
    )

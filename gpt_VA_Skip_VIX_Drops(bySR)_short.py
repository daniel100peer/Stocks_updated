...
def is_gap_valid(gap, prev_range):
    """
    מחזיר True אם גודל הגאפ תקין, False אחרת,
    על בסיס מגבלות של גודל מינימלי ומקסימלי אחוזי ביחס לטווח היום הקודם.
    """
    gap_abs = abs(gap)
...
def check_gap_closure_short(curr_day, prev_close):
    """
    בפוזיציית שורט (כניסה כשהגאפ חיובי):
    אם שער הנמוך מגיע עד שער הסגירה הקודם (prev_close) – הגאפ נסגר ויוצאים משם.
    אחרת – יוצאים בסגירת היום.
    """
...
def calculate_profit_loss_short(entry_price, exit_price, position_size):
    """
    מחשב את הרווח/הפסד לפוזיציית שורט.
    """
    number_of_shares = position_size / entry_price
    return number_of_shares * (entry_price - exit_price)

# ------------------------------------------------
# 3) Precompute All Potential Short Trades
# ------------------------------------------------
def precompute_short_trades(df, vix_df, initial_investment):
    """
    בונה DataFrame של כל הפוזיציות החוקיות רק בשורט, על בסיס גאפ חיובי (gap > 0).
    """
    trades = []
    total_days = len(df)

    # מילון map מתאריך -> שער פתיחה של VIX ליום הקודם
    ...
    for i_day in range(1, total_days):
        ...
        # אם אין שער פתיחה של VIX, אי אפשר להתקדם
        if pd.isna(vix_open):
            continue
        ...
        # אסטרטגיית שורט בלבד: מעוניינים בגאפ חיובי (gap > 0)
        if gap <= 0:
            continue
        ...
        # כאן "כיוון" העסקה הוא תמיד Short
        entry_price = curr_day['Open']
        exit_price, gap_closed = check_gap_closure_short(curr_day, prev_day['Close'])
        profit_loss = calculate_profit_loss_short(entry_price, exit_price, initial_investment)
...
def precompute_drop_ratios(vix_df, maxY=10):
    """
    מחשב drop_ratios[i, Y] = אחוז הירידה ב-VIX ב-Y הימים האחרונים (ע"פ אינדקס vix_df).
    """
...
def optimize_drop_filter_by_success_rate(trades_df, df, drop_ratios, vix_date_index,
    ...
    """
    אופטימיזציה על הפילטר של ה-Drop ע"פ שיעור ההצלחה (היחס בין עסקאות רווחיות לכלל העסקאות).
    """
    results = []
...
def find_best_vix_conditions_optimized(trades_df, i_min=10.0, i_max=30.0,
                                       ...
                                       min_occurrence_count=0):
    """
    מחפש את הטווח i < j הטוב ביותר במדד ה-VIX, ומחשב את מספר העסקאות, הרווח הכולל ושיעור ההצלחה.
    """
    ...
    valid_pairs_mask = i_flat < j_flat  # רק זוגות i < j
...
def calculate_atr(df, n):
    """
    מחשב ATR על פני חלון של n ימים.
    """
    df = df.copy()
...
def optimize_atr_parameters(trades_df, df, n_values, T_values):
    """
    מבצע אופטימיזציה על ATR – נעים על ערכי n ו-T, ומחשבים את הרווח הכולל ושיעור ההצלחה.
    """
...
# ------------------------------------------------
# 8) Main Combined Function - Short Only
# ------------------------------------------------
def run_combined_drop_vix_atr_short(
    main_file, 
    vix_file, 
    ...
    # 2) Precompute All Gap Trades (Short only)
    print("\nPrecomputing all potential SHORT gap trades...")
    trades_df = precompute_short_trades(df, vix_df, initial_investment)
    if trades_df.empty:
        print("No valid SHORT gap trades. Exiting.")
        return
    print(f"Total SHORT gap trades found: {len(trades_df)}")

    # 3) Drop Filter Optimization (by SUCCESS RATE)
    print("\n=== DROP FILTER OPTIMIZATION (by Success Rate) ===")
    maxY = int(max(Y_values))
    ...
    if final_trades.empty:
        ...
        print("\n=== FINAL COMBINED RESULTS (SHORT ONLY) ===")
...
if __name__ == "__main__":
    # דוגמה להרצת האסטרטגיה בשורט בלבד
    main_file = 'D6CK32X6.csv'
    vix_file = 'VIX_History.csv'
    run_combined_drop_vix_atr_short(
        main_file,
        vix_file,
        initial_investment=16000,
        X_values=np.round(np.arange(1.0, 20.1, 0.5), 1),
